...
//
// CORREGIDO 20 jul 2026 (ronda 5) — SELECCIÓN DE CASA POR COMPLETITUD:
// parseMarketsF5 antes se detenía en la PRIMERA casa (en orden de
// prioridad) que tuviera CUALQUIER mercado válido, aunque otra casa
// revisada después tuviera los tres mercados completos. CORREGIDO: ahora
// revisa DraftKings, FanDuel y BetMGM completas antes de decidir, y gana
// la que tenga MÁS mercados F5 válidos; en empate de cantidad, gana la
// prioridad DraftKings→FanDuel→BetMGM. Solo considera esas tres casas —
// nunca una casa fuera de esa lista, aunque aparezca en la respuesta de
// la API (se auditó y descartó una variante que sí lo permitía como
// respaldo, por el riesgo de usar una casa no aprobada).
//
...
// nunca se mezclan mercados de casas distintas dentro de un mismo
// registro; fusión por clave home|away; evento no listado en /events
// conserva su línea previa; un error nuevo nunca borra un dato bueno;
// commence_time; captured_at; congelamiento 30 minutos antes del inicio;
// nunca se usa línea en vivo.
...
// Selecciona UNA sola casa por juego, revisando DraftKings, FanDuel y
// BetMGM COMPLETAS antes de decidir (ya NO se detiene en la primera casa
// que tenga algo). Gana la casa con MAS mercados F5 validos (Moneyline +
// Run Line + Total). En empate de cantidad, gana la prioridad
// DraftKings -> FanDuel -> BetMGM -- esto sale gratis del propio orden de
// recorrido de `orden` combinado con el reemplazo estrictamente ">"
// (nunca ">="), sin necesitar una comparacion de prioridad aparte.
// Nunca mezcla mercados de casas distintas.
//
// CORREGIDO 20 jul 2026 (ronda 5): esta funcion solo itera sobre `orden`
// (las tres casas aprobadas) y busca cada una DENTRO de `lista` -- nunca
// considera una casa que no este en `orden`, aunque aparezca en la
// respuesta de la API. Una version alternativa auditada en esta misma
// ronda usaba un mapa de prioridad con un valor por defecto (100) para
// casas desconocidas, lo que permitia que una casa NO aprobada ganara la
// seleccion si las tres aprobadas no traian nada esa corrida -- se
// descarto por ese riesgo real, aunque el request ya filtra
// bookmakers=draftkings,fanduel,betmgm del lado de la API.
function parseMarketsF5(bookmakers, home, away) {
  var orden = ["draftkings", "fanduel", "betmgm"];
  var lista = Array.isArray(bookmakers) ? bookmakers : [];
  ...
  for (var bo = 0; bo < orden.length; bo++) {
    ...
    for (var b = 0; b < lista.length; b++) {
      if (lista[b] && lista[b].key === orden[bo]) { bookmakerEncontrado = lista[b]; break; }
    }
    if (!bookmakerEncontrado) continue;

    var extraido = _f5ExtraerDeUnBookmaker(bookmakerEncontrado, home, away);
    var cantidad = _f5ContarMercadosValidos(extraido);
    if (cantidad === 0) continue;

    // Solo reemplaza la seleccion anterior si trae MAS mercados que ella.
    // En empate, NO reemplaza -- como `bo` recorre en orden de prioridad,
    // la casa ya elegida siempre tiene prioridad igual o mayor.
    if (mejor === null || cantidad > mejorCantidad) {
      var titulo = bookmakerEncontrado.title || bookmakerEncontrado.key;
      mejor = {
        moneylineF5: extraido.moneylineF5,
        runlineF5: extraido.runlineF5,
        totalF5: extraido.totalF5,
        bookmaker_key: bookmakerEncontrado.key,
  ...
  if (mejor.moneylineF5) mejor.moneylineF5.bookie = mejor.bookmaker_title;
  if (mejor.runlineF5) mejor.runlineF5.bookie = mejor.bookmaker_title;
  if (mejor.totalF5) mejor.totalF5.bookie = mejor.bookmaker_title;
