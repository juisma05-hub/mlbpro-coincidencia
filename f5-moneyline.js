...
function _f5MlResultado(homeRuns, awayRuns) {
  if (homeRuns > awayRuns) return "LOCAL";
  if (awayRuns > homeRuns) return "VISITANTE";

  return "EMPATE";
}

function _f5MlProbabilidadAmericana(cuota) {
  if (!_f5MlEsNumero(cuota) || cuota === 0) {
    return null;
  }
...
function _f5MlMercadoSinVig(lineaMercadoF5) {
  ...
  var cuotaHome =
    lineaMercadoF5.moneylineF5.home_price;

  var cuotaAway =
    lineaMercadoF5.moneylineF5.away_price;

  var probHome =
    _f5MlProbabilidadAmericana(cuotaHome);

  var probAway =
    _f5MlProbabilidadAmericana(cuotaAway);
...
function f5MoneyLine(
  ...
  var ventajaLocalPct = null;
  var ventajaVisitantePct = null;
